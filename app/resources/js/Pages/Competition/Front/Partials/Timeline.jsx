const Timeline = () => {
    const timelineItems = [
        {
            title: "Pendaftaran Gelombang I",
            description: "Daftar di web himpunan!",
            status: "available",
        },
        {
            title: "Technical Meeting",
            description: "Daftar di ITCC.",
            status: "closing-soon",
        },
        {
            title: "Pengumuman Pemenang",
            description: "Saatnya mengetahui siapa yang menang!",
            status: "not-available",
        },
    ];

    const getStatusColor = (status) => {
        // perlu update types ts nya
        switch (status) {
            case "available":
                return "bg-green-500";
            case "closing-soon":
                ...
                {timelineItems.map((item, idx) => (
                    ...
                    >
                        <div className="relative flex-1 h-[160px] rounded-[10px] border border-[var(--Blue-Primary,#0F114C)] bg-white flex-shrink-0">
                            {/* Status Label */}
                            <div
                                className={`absolute top-3 right-3 flex items-center gap-2 px-3 py-1 rounded-md shadow-sm ${getStatusBgColor(
                                    item.status
                                )}`}
                            >
                                <div
                            ...
                            {/* Description */}
                            <p className="text-[#5E5E5E] text-center font-rubik font-normal leading-[150%] tracking-[0.32px] text-sm sm:text-sm md:text-base lg:text-lg px-6 sm:px-4 mt-4 sm:mt-4 xs:text-sm pt-6 sm:pt-2">
                                {item.description}
                            </p>
                        </div>
