...
function Payment({ competition_registrations, payment_methods, className }) {
    const [isCopied, setIsCopied] = useState(false);
    const { data, setData, post, put, patch, errors, processing, recentlySuccessful, formData } = useForm({
        payment_status: 'Pending',
        payment_proof_path: '',
        event_id: competition_registrations.competition_id,
        user_id: competition_registrations.user_id,
        code_registration: competition_registrations.code_registration,
        total_payment: competition_registrations.total_payment,
    ...
    const onHandleSubmit = (e) => {
        ...
        post(route('dashboard.competition.payment', { id: competition_registrations.id }), {
            forceFormData: true,
            onSuccess: (success) => {
            ...
            <section className={className}>
                ...
                <div className="grid md:grid-cols-3 grid-cols-1 md:gap-0 gap-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="font-bold text-[20px] tracking-[0.03em] text-[#000000]">Rp. {competition_registrations.total_payment}</p>
                    </div>
                    <div className="flex flex-col gap-2">
                ...
                <form onSubmit={onHandleSubmit}>
                    <div className="flex justify-center flex-col gap-5 items-center mt-10">
                        ...
                        {["Requested", "Pending", "Rejected"].includes(competition_registrations.payment_status) ? (
                            <ImageUploadDashboard
                                ...
                                classNameForBG={`${competition_registrations.payment_status === "Pending" && "cursor-not-allowed bg-[#4880FF]/15"}`}
                            />
                        ) : competition_registrations.payment_status === "Verified" && (
                        ...
                        {['Requested', 'Rejected'].includes(competition_registrations.payment_status) ? (
...
Payment.layout = (page) => <DashboardLayout children={page} title="Payment" />
