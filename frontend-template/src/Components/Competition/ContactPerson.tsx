import { SideRightCircleIcon, SideLeftArrowLeftIcon, SideRightCrossIcon } from "../Icons/HeroIcons";
...
export default function ContactPerson() {
    ...
    return (
        <section className="flex flex-col items-center py-12 mb-10 relative mt-4">
            ...
            <div className="flex flex-col-reverse lg:flex-col items-center w-full">
                <div className="lg:flex-row flex-col flex items-center lg:justify-evenly justify-center container mx-auto px-4 lg:pt-10 pt-10 gap-6 pb-24">
                    <div className="rounded-xl flex flex-col lg:flex-row border-[#ACACAC] border lg:w-3/4 w-3/4 p-4 lg:pr-20 lg:justify-between items-center">
                        <img src="/image/contactIcon.png" alt="Contact Person" className="lg:w-24 lg:h-24 w-20 h-20 my-auto" />
                        <div className="flex flex-col gap-2 p-4 items-center lg:items-start">
                            {Person1.map((person) => (
                                <p key={person.name} className="font-semibold lg:text-xl text-lg text-center">{person.name}</p>
                            ))}
                            <div className="flex items-center gap-2">
                                <img src="/image/lineIcon.png" className="w-6 h-6" alt="" />
                                {Person1.map((person) => (
                                    <a href="#" key={person.idLine} className="text-sm lg:text-lg">{person.idLine}</a>
                                ))}
                            </div>
                            <div className="flex items-center gap-2">
                                <img src="/image/whatsappIcon.png" className="w-6 h-6" alt="" />
                                {Person1.map((person) => (
                                    <a href="#" key={person.phoneNumber} className="text-sm lg:text-lg">{person.phoneNumber}</a>
                                ))}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex justify-center items-center w-full">
                    <img src="/image/chatbotIcon.png" className="w-1/2 md:w-1/3 lg:w-1/4" alt="" />
                </div>
